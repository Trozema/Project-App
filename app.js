...
//I am rewriting the project.js file with the newproject items. Please let me know if this is correct or not.//
    ...
    fileHandler.writeFile('projects.js', "const projects = " + JSON.stringify(projects) + "; module.exports = projects " , (err) => {
        if (err) throw err;
...
app.delete('/api/:id', (req, res) => {
    ...
    if (found) {
      ...
      fileHandler.writeFile('projects.js', "const projects = " + JSON.stringify(projects.filter(projects => !idFilter(req)(projects))) + "; module.exports = projects " , (err) => {
        if (err) throw err;
        res.send('File created!');
    });
    } else {
